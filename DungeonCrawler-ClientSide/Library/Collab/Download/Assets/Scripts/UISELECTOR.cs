...
public class UISELECTOR : MonoBehaviour
{
    ...
    public delegate void SkillSelected(int skill);
    public static event SkillSelected SkillSelectedEvent;
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            foreach (RaycastResult result in results)
            {
                ...
                {
                    int selectedSkill = 0;
					if (result.gameObject.name.Contains("1"))
					{
                        selectedSkill = 1;

                    }
                    if (result.gameObject.name.Contains("0"))
                    {
                        selectedSkill = 0;

                    }
                    if (result.gameObject.name.Contains("2"))
                    {
                        selectedSkill = 2;

                    }
                    if (result.gameObject.name.Contains("3"))
                    {
                        selectedSkill = 3;

                    }
                    SkillSelectedEvent(selectedSkill);
                }
				if (result.gameObject.GetComponent<CharacterData>())
				{
                    Debug.Log("WE hit : + " + result.gameObject.GetComponent<CharacterData>().GetName());
				}
            }
        }
    }
